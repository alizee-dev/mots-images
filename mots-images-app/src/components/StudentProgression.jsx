...
import { getStudentTestSessions } from '../api/students'
...
// The chart + session history for one child — factored out of
// StudentDetailPage so StudentsListPage can embed the exact same thing
// directly when there's only one child (see there), instead of
// duplicating this chart logic.
export default function StudentProgression({ studentId, studentName }) {
  ...
  return (
    <>
      ...
      {!loading && (
        <>
          {/* Shown even with zero sessions — the empty chart frame itself,
              plus a message explaining what will appear here, reads as
              more inviting on first discovery than hiding this whole
              section until a first évaluation is completed. */}
            ...
            {chartLoading ? (
              ...
              <>
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={[]} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
                ...
                <p className="empty-hint">
                  La progression de {studentName} apparaîtra ici après sa première évaluation.
                </p>
              </>
            ) : chartData.length > 0 ? (
              <>
